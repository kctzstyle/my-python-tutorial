...
# 세트; 셋(set): 수학에서의 `집합`과 동일한 의미를 가집니다.
#                중복된 값을 가지지 않으며 순서를 갖지 않습니다.
...
# add: 세트에 요소를 추가시킵니다.
#      리스트의 `append`와 동일한 역할을 하지만, 세트는 순서를 가지지 않으므로 추가 시 오름차순으로 자동 정렬됩니다.
s.add(6)
...
# discard: `remove`와 동일하지만, `remove`는 사용 시 해당하는 요소가 없으면 `KeyError`를 발생하는 반면,
...
# symmetric_difference : 수학에서의 대칭차집합(합집합 - 교집합) 연산자와 동일합니다.
...
# issubset: 해당 이터러블에 대한 부분집합(subset)인지 여부를 확인합니다.
