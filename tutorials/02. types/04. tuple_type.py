...
# tuple: list와 마찬가지로 순서가 있는 컨테이너 타입입니다.
#        단, list처럼 안의 요소를 변경할 수 없습니다.

t = ()
# <class 'tuple'>
print(type(t))

# list처럼 값을 변경하는 기능은 없기 때문에
# append, pop, insert, remove 등의 기능을 사용할 수는 없지만
# count, index의 기능은 정의되어 있습니다.
# 이 함수들은 list에서의 count, index와 역할도 같습니다.
...
# next: 튜플에 저장되어 있는 다음 값을 불러오며 메모리에서 해제하는 함수입니다.
#       순서대로 값을 불러오다가 더 이상 값이 존재하지 않으면 StopIteration 예외를 발생시킵니다.
#       리스트의 pop과 비슷해 보이지만, 성능면에서 뛰어난 면을 보여줍니다.
